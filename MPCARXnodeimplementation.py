...
class MPC:
    ...
    def initSimProperty(self):
        ...
        self.opt_buoyancy_engine = np.array([])
        self.estimated_depth_rate = np.array([])
        self.opt_movingmass= np.array([])
    ...
    def plot(self):
        ...
        self.opt_buoyancy_engine = np.append(self.opt_buoyancy_engine,(self.current_be/100))
        self.estimated_depth_rate = np.append(self.estimated_depth_rate, (self.depth_rate.VALUE[index]/1000))
        self.opt_movingmass = np.append(self.opt_movingmass, self.current_mm/100)
        ...
        # plt.figsize()
        plt.subplot(4, 1, 1)
        plt.title("MPC Control Implementation".format(self.max_depth_rate))
        print(self.opt_moving_mass)
        plt.plot(time[0:self.time], self.opt_moving_mass[0:self.time],
                label='Moving Mass Rate')
        plt.plot(time[0:self.time], self.opt_buoyancy_engine_rate[0:self.time], label='Buoyancy Engine Rate')
        plt.plot(time[0:self.time], self.opt_buoyancy_engine[0:self.time], label='Buoyancy Engine')
        plt.plot(time[0:self.time], self.opt_movingmass[0:self.time], label="Moving Mass")
        plt.ylabel('Input')
        plt.legend()
        ...
        plt.plot(time[0:self.time], self.estimated_depth_rate[0:self.time], color='r',
                linestyle=':', label='Estimated Depth Rate')
        plt.xlabel('Time (s)')
    ...
    def mainProgram(self):
        ...
        # error varying weight
        
        self.R1=2
        self.depth_rate_error=self.current_depth_rate - self.depth_rate_sp
        if(abs(self.depth_rate_error)<0.05):
            self.R1=10
        if(abs(self.depth_rate_error)<0.04):
            self.R1=50
        ...
        if(abs(self.pitch_rate_error)<6):
            self.R2=0.001
        if(abs(self.pitch_rate_error)<5):
            self.R2=0.005
        if(abs(self.pitch_rate_error)<3):
            self.R2=0.1
        if(abs(self.pitch_rate_error)<2):
            self.R2=1
        # if(abs(self.pitch_rate_error)<1):
        #     self.R2=100
        ...
        self.m.Minimize(1*((self.pitch_angle - self.pitch_sp)/20)**2)
